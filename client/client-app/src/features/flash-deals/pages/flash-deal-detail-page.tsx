...
import { DealImageCarousel } from "@/features/flash-deals/components/deal-image-carousel";
import { DealProductsCarousel } from "@/features/flash-deals/components/deal-products-carousel";
import {
  Heart,
  Share2,
  ChevronRight,
...
export function FlashDealDetailPage() {
  const { id } = useParams({ strict: false });
  
  const deal = deals.find((d) => d.id.toString() === id);
      ...
      <section className="mb-10 grid grid-cols-1 gap-8 lg:grid-cols-2 xl:grid-cols-12">
        ...
        <div className="flex flex-col justify-center lg:col-span-1 xl:col-span-4">
          ...
          <div className="mb-6 flex flex-col items-center justify-between gap-4 rounded-2xl border border-purple-100 bg-purple-50 p-4 sm:flex-row">
            ...
            <Link
              to="/flash-deals/register/$id" params={{ id: deal?.id?.toString() || "" }}
              className="bg-primary shadow-primary/20 flex w-full items-center justify-center whitespace-nowrap rounded-xl px-6 py-3 text-sm font-bold text-white shadow-md transition hover:opacity-90 sm:w-auto"
            >
              Register for ₹1
            </Link>
          </div>
        ...
        <div className="fixed bottom-0 left-0 right-0 z-50 translate-y-0 transform border-t border-gray-200 bg-white shadow-[0_-4px_20px_-10px_rgba(0,0,0,0.1)] transition-transform duration-300">
          <div className="mx-auto flex max-w-[1920px] flex-col items-center justify-between gap-4 px-4 py-4 md:flex-row md:px-8 xl:px-10">
            ...
            <div className="mt-4 flex w-full items-center gap-2 sm:gap-3 md:mt-0 md:w-auto">
              <Link
                to="/flash-deals/register/$id" params={{ id: deal?.id?.toString() || "" }}
                className="bg-primary shadow-primary/20 flex flex-1 items-center justify-center whitespace-nowrap rounded-xl px-2 py-3.5 text-[13px] font-bold text-white shadow-md transition hover:opacity-90 sm:px-8 sm:text-sm md:flex-none"
              >
