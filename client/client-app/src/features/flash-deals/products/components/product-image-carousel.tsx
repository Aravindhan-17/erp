      ...
      <div className="flex flex-row gap-3 overflow-x-auto md:flex-col">
        {images.map((img, index) => (
          <button
            ...
            `}
          >
            <img src={img} alt={title} className="absolute inset-0 h-full w-full object-contain p-2" />
          </button>
        ))}
      </div>

      {/* main image */}
      <div className="h-87.5 md:h-130 group relative flex items-center justify-center overflow-hidden rounded-3xl border border-gray-200 bg-white">
        {/* Embla Viewport */}
