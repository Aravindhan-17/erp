...
import { Heart, ShoppingCart, Trash2, MoreHorizontal, Share2 } from "lucide-react";
    ...
    <div className="flex flex-col gap-8">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-gray-200 pb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Keep track of the items you love and move them to your cart when you're ready.</p>
        </div>
      ...
      {wishlistItems.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-gray-200 bg-gray-50 py-16 text-center">
          ...
          <p className="mt-2 text-sm text-gray-500">Find something you love and tap the heart icon to save it here.</p>
          <Link href="/" className="mt-6 rounded-xl bg-primary px-6 py-2.5 text-sm font-semibold text-white transition-opacity hover:opacity-90">
            Start Shopping
          </Link>
        </div>
      ) : (
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {wishlistItems.map((item) => (
            <div key={item.id} className="group relative flex flex-col overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm transition-all hover:border-secondary hover:shadow-md">
              {/* Product Image */}
              <div className="relative h-48 w-full bg-gray-50 p-6 flex items-center justify-center border-b border-gray-100">
                <Image 
                  src={item.image} 
                  alt={item.name} 
                ...
                {!item.inStock && (
                  <div className="absolute inset-0 flex items-center justify-center bg-white/30 backdrop-blur-sm">
                    <span className="rounded-full bg-white/90 px-3 py-1 text-xs font-bold uppercase tracking-wider text-gray-900 shadow-sm ring-1 ring-inset ring-gray-900/10 backdrop-blur-md">Out of Stock</span>
                  </div>
                )}
              </div>
              ...
              <div className="flex flex-1 flex-col p-5">
                <div className="mb-2 flex items-start justify-between gap-2">
                  <Link href={`/product/${item.id}`} className="font-bold text-gray-900 line-clamp-2 hover:text-secondary">
                    {item.name}
                  </Link>
                </div>
                
                <p className="text-xs text-gray-500 mb-4">Added {item.addedDate}</p>
