...
const customers = [
  {
    ...
    recentOrders: [
      { deal: "Office Essentials Clearance", id: "FD-88192", date: "02 Aug", amount: "₹5,999", status: "Delivered" }
    ],
    lifetimeSpend: "₹5,999",
    dealsJoined: 1,
    avgOrderValue: "₹5,999"
  },
  {
    ...
    recentOrders: [
      { deal: "Monsoon Electronics Sale", id: "FD-88231", date: "20 Jul", amount: "₹52,999", status: "Confirmed" }
    ],
  ...
  },
  {
    ...
    location: "New Delhi",
    recentOrders: [
      { deal: "Monsoon Electronics Sale", id: "FD-88230", date: "20 Jul", amount: "₹10,998", status: "Shipped" }
    ],
  ...
  },
  {
    ...
    recentOrders: [
      { deal: "Office Essentials Clearance", id: "FD-88229", date: "19 Jul", amount: "₹5,999", status: "Delivered" }
    ],
    lifetimeSpend: "₹5,999",
    dealsJoined: 1,
    avgOrderValue: "₹5,999"
  },
  {
    ...
    recentOrders: [
      { deal: "Monsoon Electronics Sale", id: "FD-88228", date: "20 Jul", amount: "₹41,999", status: "Payment Pending" }
    ],
    lifetimeSpend: "₹0",
    dealsJoined: 1,
    avgOrderValue: "₹0"
  },
  {
    ...
    recentOrders: [
      { deal: "Office Essentials Clearance", id: "FD-88227", date: "19 Jul", amount: "₹3,398", status: "Cancelled" }
    ],
    lifetimeSpend: "₹3,398",
    dealsJoined: 1,
    avgOrderValue: "₹3,398"
  ...
  return (
    <div className="min-w-0 overflow-x-hidden font-poppins">
      ...
      <div className="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight text-gray-900 sm:text-3xl">Customers</h1>
          <p className="mt-1 text-sm text-gray-500">Customer profiles, participation, spend and risk signals.</p>
        </div>
        <button type="button" className="flex w-fit items-center gap-2 rounded-xl border border-gray-200 bg-white px-4 py-2.5 text-sm font-semibold text-gray-800 shadow-sm transition-all hover:border-gray-300 hover:shadow">
          ...
          <table className="w-full min-w-275 border-collapse">
            <thead>
              <tr className="border-b border-gray-100">
                ...
                <th className="px-4 py-4 text-left text-[12px] font-semibold uppercase tracking-wide text-gray-400">Total Spend</th>
                <th className="px-4 py-4 text-left text-[12px] font-semibold uppercase tracking-wide text-gray-400">Last Activity</th>
                <th className="px-4 py-4 text-left text-[12px] font-semibold uppercase tracking-wide text-gray-400">Status</th>
                <th className="px-4 py-4 text-center text-[12px] font-semibold uppercase tracking-wide text-gray-400">Actions</th>
              </tr>
            </thead>
              ...
              {customers.map((customer) => (
                <tr key={customer.id} className="border-b border-gray-100 transition-colors hover:bg-primary/5">
                  ...
                  <td className="px-4 py-4 text-center">
                    <span className="text-sm font-medium text-gray-700">{customer.registrations}</span>
                  </td>
                  <td className="px-4 py-4 text-center">
                    <span className="text-sm font-medium text-gray-700">{customer.orders}</span>
                  </td>
                  <td className="px-4 py-4">
                    <span className="whitespace-nowrap text-sm font-medium text-gray-700">{customer.spend}</span>
                  </td>
                  <td className="px-4 py-4">
                    <span className="whitespace-nowrap text-xs font-medium text-gray-400">{customer.activity}</span>
                  </td>
                  <td className="px-4 py-4">
                    <span className={`inline-flex whitespace-nowrap rounded-full px-3 py-1.5 text-[12px] font-semibold ${
                      customer.status === "Review" ? "bg-amber-50 text-amber-600" : "bg-emerald-50 text-emerald-600"
                    }`}>
                      {customer.status}
                    </span>
                  </td>
          ...
          <div className="space-y-8 mt-4">
            ...
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
              <div className="rounded-xl border border-gray-100 bg-gray-50/50 p-4">
                <p className="text-[10px] font-bold uppercase tracking-wider text-gray-400">Lifetime Spend</p>
                <p className="mt-2 text-xl font-bold text-gray-900">{viewingCustomer.lifetimeSpend}</p>
              </div>
              <div className="rounded-xl border border-gray-100 bg-gray-50/50 p-4">
                <p className="text-[10px] font-bold uppercase tracking-wider text-gray-400">Flash Deals Joined</p>
                <p className="mt-2 text-xl font-bold text-gray-900">{viewingCustomer.dealsJoined}</p>
              </div>
              <div className="rounded-xl border border-gray-100 bg-gray-50/50 p-4">
              ...
              <div className="rounded-xl border border-gray-100 bg-gray-50/50 p-4">
                <p className="text-[10px] font-bold uppercase tracking-wider text-gray-400">Avg. Order Value</p>
                <p className="mt-2 text-xl font-bold text-gray-900">{viewingCustomer.avgOrderValue}</p>
              </div>
            </div>

            {/* Recent Orders List */}
            <div>
              <p className="mb-4 text-[11px] font-bold uppercase tracking-wider text-gray-500">Recent Orders</p>
              ...
              <div className="overflow-hidden rounded-xl border border-gray-100 bg-white">
                <div className="overflow-x-auto">
                    ...
                    <tbody className="divide-y divide-gray-100">
                      {(viewingCustomer.recentOrders as { deal: string; id: string; date: string; amount: string; status: string }[])?.map((order, idx: number) => (
                        <tr key={idx}>
                          ...
                          <td className="px-4 py-3 font-medium text-gray-900 whitespace-nowrap">{order.amount}</td>
                  ...
                  {(!viewingCustomer.recentOrders || viewingCustomer.recentOrders.length === 0) && (
                     <div className="py-6 text-center text-sm text-gray-500">No recent orders found.</div>
                  )}
                </div>
              </div>
            </div>
