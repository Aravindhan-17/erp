  ...
  return (
    <div className="w-full min-w-0 space-y-5 sm:space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <button
          type="button"
          className="bg-primary hover:bg-primary-hover shadow-primary/20 inline-flex w-fit items-center gap-2 rounded-xl px-4 py-2.5 text-sm font-semibold text-white shadow-md transition hover:-translate-y-0.5 hover:shadow-md"
        >
          <Plus size={18} />
          Create new deal
        </button>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {stats.map((stat) => {
          ...
          return (
            <div
              key={stat.title}
              className="group rounded-2xl border border-gray-100 bg-white p-5 shadow-[0_2px_12px_rgba(30,20,80,0.04)] transition duration-200 hover:-translate-y-0.5 hover:shadow-[0_8px_25px_rgba(30,20,80,0.07)]"
            >
              ...
              <div className="flex h-full min-w-0 flex-1 items-end justify-between gap-1 sm:gap-4">
                {revenue.map((item) => (
                  ...
                  >
                    ...
                    <div
                      className="max-w-14.5 bg-linear-to-t from-primary/20 to-primary w-full rounded-t-md"
                      style={{
          ...
          <div className="mt-5 divide-y divide-gray-100">
            {activeDeals.map((deal) => (
              <div key={deal.title} className="flex items-center gap-3 py-3 first:pt-0 last:pb-0">
                ...
                <span
                  className={`shrink-0 rounded-full px-3 py-1.5 text-[10px] font-bold ${deal.statusClass}`}
                >
                  {deal.status}
                </span>
              </div>
      ...
      <button
        type="button"
        className="fixed bottom-5 right-5 z-30 flex items-center gap-2 rounded-full bg-[#172033] px-4 py-3 text-xs font-semibold text-white shadow-xl transition hover:-translate-y-0.5 hover:bg-[#111827]"
      >
